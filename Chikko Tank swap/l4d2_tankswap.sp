...
public TS_MenuCallBack(Handle:menu, MenuAction:action, param1, param2)
{
    ...
    if (!choice)
    {
        ...
        CPrintToChatAll("{default}[{red}Chikko's zonemod{default}] {green}坦克控制权已经转移给: {red}%N", choice);
    ...
    else
    {
        ...
        CPrintToChatAll("{default}[{red}YJ'zonemod{default}] {green}坦克控制权已经转移给: {red}%N", choice);
...
public Action:TS_Display_Auto_MenuToTank(Handle:timer)
{
    ...
    SetMenuTitle(surrenderMenu, " Chikko's zonemod 坦克控制权面板 ");
